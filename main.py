...
if __name__ == "__main__":
    ...
    true_map = combine.single_color_image(0, 0, 0)
    last_image = true_map
    first_image = True
    ...
    for filename in sorted_filenames:
        ...
        if filename != "EEB_Transform.txt":
            ...
            # preprocess image
            #map_piece = make_threshold(map_piece, write_picture=True)
            ...
            if not first_image:
                blank_canvas = combine.single_color_image(map_size_x + delta_x, map_size_y + delta_y, 0)
                map_size_x += delta_x
                map_size_y += delta_y
            else:
                blank_canvas = combine.single_color_image(map_size_x, map_size_y, 0)
                first_image = False
